...
class ExpertDataset(object):
    ...
    def __init__(self, expert_path=None, traj_data=None, train_fraction=0.7, batch_size=64,
        ...
        new_observations = traj_data['new_obs'][:traj_limit_idx]
        ...
        self.train_loader = DataLoader(train_indices, self.observations, self.actions, self.new_observations,
                                       self.rewards, self.dones, batch_size,
                                       shuffle=self.randomize, start_process=False,
                                       sequential=sequential_preprocessing)
        self.val_loader = DataLoader(val_indices, self.observations, self.actions, self.new_observations,
                                     self.rewards, self.dones, batch_size,
                                     shuffle=self.randomize, start_process=False,
                                     sequential=sequential_preprocessing)
...
class DataLoader(object):
    ...
    def __init__(self, indices, observations, actions, new_observations, rewards, dones, batch_size, n_workers=1,
        ...
        self.observations = observations
        # self.new_observations = new_observations
        self.next_obs = new_observations
        self.rewards = rewards
        self.dones = dones
        self.actions = actions
        ...
        with Parallel(n_jobs=self.n_workers, batch_size="auto", backend=self.backend) as parallel:
            while start or self.infinite_loop:
                ...
                for minibatch_idx in range(self.n_minibatches):
                    ...
                    self.queue.put((obs, actions, rewards, next_obs, dones))
