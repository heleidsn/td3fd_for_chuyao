...
class TD3(OffPolicyRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("input", reuse=False):
                    ...
                    self.bc_loss_weight_ph = tf.placeholder(tf.float32, [], name="bc_loss_weight_ph")
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.bc_loss = tf.reduce_mean(tf.square(policy_out - self.expert_actions_ph))
                    self.policy_loss_pretrain = -tf.reduce_mean(qf1_pi) + self.bc_loss * self.bc_loss_weight_ph
                    ...
                    policy_train_op = policy_optimizer.minimize(self.policy_loss_pretrain, var_list=get_vars('model/pi'))
                    self.policy_train_op = policy_train_op

                    # add policy_pretrain_op with bc loss for pretrain
                    bc_optimizer = tf.train.AdamOptimizer(learning_rate=1e-4)
                    self.policy_pretrain_op = bc_optimizer.minimize(self.policy_loss_pretrain, var_list=get_vars('model/pi'))
                    self.policy_pretrain_bc_op = bc_optimizer.minimize(self.bc_loss, var_list=get_vars('model/pi'))
                    ...
                    tf.summary.scalar('policy_loss_pretrain', self.policy_loss_pretrain)
                    tf.summary.scalar('bc_loss_weight_ph', self.bc_loss_weight_ph)
                    tf.summary.histogram('policy_dense_bias', get_vars("model/pi/dense/bias"))
                    tf.summary.histogram('policy_dense_kernel', get_vars("model/pi/dense/kernel"))
                ...
                if self.tensorboard_log != None:
                    summary_writer = tf.summary.FileWriter(self.tensorboard_log, self.graph)
    ...
    def _train_step(self, step, writer, learning_rate, update_policy):
        ...
        total_step = 5000
        bc_loss_weight_ph = 20 * (total_step - step)/total_step

        if bc_loss_weight_ph < 0:
            bc_loss_weight_ph = 0
        ...
        feed_dict = {
            ...
            self.expert_actions_ph: batch_actions,
            self.bc_loss_weight_ph: bc_loss_weight_ph
        }
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                if done:
                    ...
                    maybe_step_num = info.get('step_num')

                    # add tensorboard summary
                    if writer is not None:
                        summary_success = tf.Summary(value=[tf.Summary.Value(tag="info/mean 20 episode success rate", simple_value=np.mean(episode_successes[-20:]))])
                        summary_success_100 = tf.Summary(value=[tf.Summary.Value(tag="info/mean 100 episode success rate", simple_value=np.mean(episode_successes[-100:]))])
                        summary_crash = tf.Summary(value=[tf.Summary.Value(tag="info/mean 20 episode crash rate", simple_value=np.mean(episode_crashes[-20:]))])
                        summary_crash_100 = tf.Summary(value=[tf.Summary.Value(tag="info/mean 100 episode crash rate", simple_value=np.mean(episode_crashes[-100:]))])
                        summary_step_num = tf.Summary(value=[tf.Summary.Value(tag="info/episode step_num", simple_value=maybe_step_num)])
                        writer.add_summary(summary_success, self.num_timesteps)
                        writer.add_summary(summary_success_100, self.num_timesteps)
                        writer.add_summary(summary_crash, self.num_timesteps)
                        writer.add_summary(summary_crash_100, self.num_timesteps)
                        writer.add_summary(summary_step_num, self.num_timesteps)
    ...
    def save(self, save_path, save_replay_buffer=False, cloudpickle=False):
        data = {
    ...
    def generate_expert_replay_buffer(self, total_episode_num):
        """
        generate replay buffer using expert action
        replay_buffer
        """
        # TODO: add success and crash rate
        obs = self.env.reset()
        ...
        while episode < total_episode_num:
            # get expert action
            action = self.env.get_avoidance_output_state()
            unscaled_action = unscale_action(self.env.action_space, action)
            # interate with envrionment
            new_obs, reward, done, info = self.env.step(unscaled_action)
            ...
            if done:
                ...
                episode_step_num.append(float(info.get('step_num')))
                
                print("episode: ", episode, " Reward: ", episode_reward[-1], "Step num: ", episode_step_num[-1])
                if episode == total_episode_num:
                    break
                else:
                    obs = self.env.reset()
        ...
        print("get {:d} expert trajectories, average reward: {:.2f}, buffersize: {:d}".format(total_episode_num,\
                                                                    average_reward,\
                                                                    len(self.replay_buffer)))

        print("Success rate: {:.2f} Crash rate: {:.2f} Average episode step: {:.2f}".format(np.mean(episode_successes),
                                                                    np.mean(episode_crashes), np.mean(episode_step_num)))
                    
    def pretrain(self, total_timesteps, learning_rate=3e-4, callback=None,
                    log_interval=4, tb_log_name="TD3_pretrain", reset_num_timesteps=True, replay_wrapper=None):
        new_tb_log = self._init_num_timesteps(reset_num_timesteps)
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                # Sample a batch from the expert replay buffer 
                batch = self.replay_buffer.sample(self.batch_size)
                batch_obs, batch_actions, batch_rewards, batch_next_obs, batch_dones = batch
                ...
                feed_dict = {
                    ...
                    self.learning_rate_ph: learning_rate,
                    self.expert_actions_ph: batch_actions,
                    self.bc_loss_weight_ph: 20
                }

                step_ops = self.step_ops
                step_ops = step_ops + [self.policy_pretrain_op, self.target_ops, self.policy_loss, self.bc_loss]

                # add tensorboard log
                if writer is not None:
                    out = self.sess.run([self.summary] + step_ops, feed_dict)
                    parameters = self.get_parameter_list()
                    policy_dense_kernel = self.sess.run(parameters[12])
                    policy_dense_bias = self.sess.run(parameters[13])
                    print('Train step:', step, 'BC loss: ', out[-1], 'bias:', policy_dense_bias)
                    summary = out.pop(0)
                    writer.add_summary(summary, step)
                else:
                    out = self.sess.run(step_ops, feed_dict)    
    ...
    def pretrain_pure_bc(self, total_timesteps, learning_rate=3e-4, callback=None,
                    log_interval=4, tb_log_name="TD3_pretrain_purebc", reset_num_timesteps=True, replay_wrapper=None):
        new_tb_log = self._init_num_timesteps(reset_num_timesteps)
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                step_ops = [self.policy_pretrain_bc_op, self.policy_out, self.bc_loss]
                ...
                if writer is not None:
                    ...
                    # policy_dense_kernel = self.sess.run(parameters[12])
                    # policy_dense_bias = self.sess.run(parameters[13])
                    print('Train step:', step, 'BC loss: ', out[-1])
                    summary = out.pop(0)
                    writer.add_summary(summary, step)
                else:
                    out = self.sess.run(step_ops, feed_dict)    
